...
while 1:
    ...
    for ID in XM_ID:
        packetHandler.write2ByteTxRx(portHandler, ID, ADDR_XM_GOAL_POSITION, 2700)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
    ...
    for ID in XM_ID:
        present_position = packetHandler.read2ByteTxRx(portHandler, ID, ADDR_XM_PRESENT_POSITION)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
    ...
    print("-")
    print("[ID:%03d] GoalPos:%03d  PresPos:%s [ID:%03d] GoalPos:%03d  PresPos:%s [ID:%03d] GoalPos:%03d  PresPos:%s" 
        % (FR1_ID, goal_position[index], present_position[0], FR2_ID, goal_position[index], present_position[0], FR3_ID, goal_position[index], present_position[0]))

    print("[ID:%03d] GoalPos:%03d  PresPos:%s [ID:%03d] GoalPos:%03d  PresPos:%s [ID:%03d] GoalPos:%03d  PresPos:%s" 
        % (FL4_ID, goal_position[index], present_position[0], FL5_ID, goal_position[index], present_position[0], FL6_ID, goal_position[index], present_position[0]))

    print("[ID:%03d] GoalPos:%03d  PresPos:%s [ID:%03d] GoalPos:%03d  PresPos:%s [ID:%03d] GoalPos:%03d  PresPos:%s" 
        % (RR7_ID, goal_position[index], present_position[0], RR8_ID, goal_position[index], present_position[0], RR9_ID, goal_position[index], present_position[0]))

    print("[ID:%03d] GoalPos:%03d  PresPos:%s [ID:%03d] GoalPos:%03d  PresPos:%s [ID:%03d] GoalPos:%03d  PresPos:%s" 
        % (RL10_ID, goal_position[index], present_position[0], RL11_ID, goal_position[index], present_position[0], RL12_ID, goal_position[index], present_position[0]))
    present_position_int = present_position[0]
    print(present_position_int)
    print("-")
    if not (abs(2700 - present_position_int) > MOVING_STATUS_THRESHOLD):
        break
